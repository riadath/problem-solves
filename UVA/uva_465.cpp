...
char mxc[] = "2147483647";
int compare(char ch1[]){
    // puts(ch1);
    int st = 0,tlen = strlen(ch1),len2 = 10;
    while(ch1[st] == '0' && st < tlen){
        st++;
    }
    int len1 = 0;
    for(int i = st;ch1[i] != 0 && i < tlen;i++)len1++;
    // printf("%d %d <<<<<<\n",st,len1);
    if(len1 == len2){
        for(int i = st,j = 0;i < tlen && j < len2;i++,j++){
            if(ch1[i] != mxc[j]){
                return (ch1[i] < mxc[j]);
            }
        }
...
int main()
{
    ...
    char num1[1000],ch,num2[1000];
    while(scanf("%s %c %s",num1,&ch,num2) == 3){
        BigInt a,b;
        a.init(num1),b.init(num2);
        if(ch == '+')a.add(b);
        if(ch == '*')a.mul(b);
        printf("%s %c %s\n",num1,ch,num2);
        if(!compare(num1)){
